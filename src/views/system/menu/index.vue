      ...
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        ...
        <el-form-item>
          <el-button type="primary" @click="handleQuery"
            ><template #icon><svg-icon icon-class="search" /></template>搜索</el-button
          >
          <el-button @click="resetQuery">
            <template #icon><svg-icon icon-class="reset" /></template>
            重置</el-button
          >
        </el-form-item>
      </el-form>
    ...
    <el-card shadow="never" class="table-container">
      <template #header>
        <el-button v-hasPerm="['sys:menu:add']" type="success" @click="openDialog(0)">
          <template #icon><svg-icon icon-class="plus" /></template>
          新增</el-button
        >
      </template>
      ...
      >
        ...
        <el-table-column label="类型" align="center" width="80">
          <template #default="scope">
            <el-tag v-if="scope.row.type === NodeTypeEnum.CATALOG" type="warning">目录</el-tag>
            <el-tag v-if="scope.row.type === NodeTypeEnum.MENU" type="success">菜单</el-tag>
            <el-tag v-if="scope.row.type === NodeTypeEnum.BUTTON" type="danger">按钮</el-tag>
            <el-tag v-if="scope.row.type === NodeTypeEnum.EXTLINK" type="info">外链</el-tag>
          </template>
        </el-table-column>
    ...
    >
      <el-form ref="menuFormRef" :model="formData" :rules="rules" label-width="160px">
        ...
        <el-form-item label="菜单类型" prop="type">
          <el-radio-group v-model="formData.type" @change="onNodeTypeChange">
        ...
        <el-form-item v-if="formData.type == NodeTypeEnum.EXTLINK" label="外链地址" prop="path">
          <el-input v-model="formData.path" placeholder="请输入外链完整路径" />
        </el-form-item>

        <el-form-item
          v-if="formData.type == NodeTypeEnum.CATALOG || formData.type == NodeTypeEnum.MENU"
          label="路由路径"
          prop="path"
        >
          <el-input
        ...
        <!-- 组件页面完整路径 -->
        <el-form-item v-if="formData.type == NodeTypeEnum.MENU" label="页面路径" prop="component">
          <el-input v-model="formData.component" placeholder="system/user/index" style="width: 95%">
            <template v-if="formData.type == NodeTypeEnum.MENU" #prepend>src/views/</template>
            <template v-if="formData.type == NodeTypeEnum.MENU" #append>.vue</template>
          </el-input>
        </el-form-item>

        <el-form-item v-if="formData.type !== NodeTypeEnum.BUTTON" prop="visible" label="显示状态">
        ...
        <el-form-item v-if="formData.type === NodeTypeEnum.CATALOG" label="根目录始终显示">
          <template #label>
            <div>
              根目录始终显示
        ...
        <el-form-item v-if="formData.type === NodeTypeEnum.MENU" label="是否缓存">
        ...
        <!-- 权限标识 -->
        <el-form-item v-if="formData.type == NodeTypeEnum.BUTTON" label="权限标识" prop="perm">
          <el-input v-model="formData.perm" placeholder="sys:user:add" />
        </el-form-item>

        <el-form-item v-if="formData.type !== NodeTypeEnum.BUTTON" label="图标" prop="icon">
          <!-- 图标选择器 -->
          <icon-select v-model="formData.icon" />
        </el-form-item>

        <el-form-item v-if="formData.type == NodeTypeEnum.CATALOG" label="跳转路由">
...
import { listMenus, getMenuForm, getMenuOptions, addMenu, deleteMenu, updateMenu } from '@/api/menu'
