...
import { usePermissionStore, useTabviewStore, useSettingsStore, useGlobalStore } from '@/stores'
...
function initTags() {
  ...
  for (const tag of tags) {
    // Must have tag name
    if (tag.name) {
      tabviewStore.addVisitedView(tag)
    }
  }
}
...
function moveToCurrentTag() {
  // 使用 nextTick() 的目的是确保在更新 Tabview 组件之前，scrollPaneRef 对象已经滚动到了正确的位置。
  nextTick(() => {
...
function toLastView(visitedViews, view) {
  ...
  if (latestView && latestView.fullPath) {
    ...
    // now the default is to redirect to the home page if there is no tabs,
...
function closeSelectedTag(view) {
  tabviewStore.delView(view).then((res) => {
    if (isActive(view)) {
...
function closeLeftTags() {
  tabviewStore.delLeftViews(selectedTag.value).then((res) => {
...
function closeRightTags() {
  tabviewStore.delRightViews(selectedTag.value).then((res) => {
    if (!res.visitedViews.find((item) => item.path === route.path)) {
      toLastView(res.visitedViews)
    }
  })
...
function closeOtherTags() {
  ...
  tabviewStore.delOtherViews(selectedTag.value).then(() => {
    moveToCurrentTag()
  })
}

function closeAllTags(view) {
  tabviewStore.delAllViews().then((res) => {
    toLastView(res.visitedViews, view)
  })
}
